...
Explicitement écarté de cette v1 pour ce tour : le routage de phase piloté par LLM (un appel LLM avant chaque transition, borné par les mêmes garde-fous) — probablement une part importante du "~95% du temps en inférence" de la section 5, et le pipeline linéaire déterministe de v2 reste le choix retenu. Le remplacement de `evasion_level` (hors périmètre MVP, voir `PROJECT.md`) et de la config spéculative (`hydra_path`, `masscan_path`, etc., sans outil correspondant) n'a pas non plus été porté.
