...
- [ ] `gobuster`, `ffuf`, `nuclei`, `dalfox` : binaires de release GitHub (multi-arch `amd64`/`arm64`), pas de build depuis les sources. Vérifier individuellement, pour chaque outil, à la fois le nom de l'asset (schémas déjà tous différents : `.tar.gz` vs `.zip`, `x86_64` vs `amd64`, `arm64` vs `aarch64`, avec ou sans le `v` du tag) **et** la structure interne de l'archive (`dalfox` imbrique son binaire dans un sous-dossier versionné, contrairement aux trois autres qui l'ont à la racine — `tar --strip-components=1` plutôt qu'un nom de fichier fixe pour ne pas en dépendre). Ne jamais supposer qu'un outil suit le même schéma qu'un autre du même Dockerfile, sur aucun des deux points.
- [ ] Avant d'écrire un parser pour un outil externe : vérifier son langage d'implémentation actuel (`gh api repos/<owner>/<repo> --jq '.language'`) et lire le code source réel du champ qui distingue un résultat positif d'un résultat négatif/informationnel, pas seulement sa documentation générale ou ses flags CLI. Un outil peut avoir changé de langage (et donc de format de sortie) entre la documentation trouvée en ligne et la version réellement installée (`dalfox` : réécriture Go → Rust, champ `"type"` à quatre valeurs dont une seule - `"V"` - est une confirmation réelle, les autres semblaient à tort être des confirmations aussi).
...
- [ ] L'outil qui teste la sécurité d'autrui doit recevoir le même niveau d'exigence sur sa propre surface d'exposition : toute route `/api/*` passe par `require_api_key` (vide par défaut pour ne pas casser un déploiement existant, mais un avertissement est journalisé au démarrage tant que `API_KEY` n'est pas définie) ; tout champ texte libre stocké dans un `Finding`/`Lead` passe par la redaction du cookie de session avant stockage (`MissionState.add_finding`/`add_lead`), jamais une garantie supposée du seul fait qu'un outil (nuclei) redige lui-même sa propre sortie (`docs/HISTORY.md`, section 20).
...
| `REQUIRE_AUTHORIZATION` | `true` | Bloque toute mission sans `authorization_ref`. Verifie seulement qu'une chaine non vide est fournie - une note pour la tracabilite, pas un controle technique (voir `API_KEY` pour la protection reelle de l'API). |
| `API_KEY` | (vide) | Cle partagee protegeant chaque route `/api/*` (en-tete `X-API-Key`, ou `?api_key=` en repli pour le lien de telechargement direct du rapport). Vide par defaut = API ouverte a quiconque atteint le port - a definir avant toute exposition au-dela de la machine de l'operateur (`docker-compose.yml` publie sur `0.0.0.0` par defaut). Voir `docs/HISTORY.md`, section 20. |
...
- Décider si les tests de la section 11 doivent tourner en CI (GitHub Actions) dès le MVP ou seulement en local dans un premier temps. **Tranché** : oui, dès que le dépôt est devenu public — `.github/workflows/tests.yml` lance `pytest` sur chaque push/PR vers `main`. Se limite volontairement aux tests unitaires (`tests/`), qui ne nécessitent ni Ollama ni les binaires d'outils externes ; aucune mission de bout en bout contre une cible réelle n'est exécutée en CI.
